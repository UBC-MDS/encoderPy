...
true_train2_mean = pd.Series(np.concatenate((np.repeat(0.392902, repeats=10),
                                   np.repeat(-0.401635, repeats=10))))
...
def test_check_regression():
    ...
    with pytest.raises(Exception):
        conjugate_encoder.conjugate_encoder(X_train = train2.iloc[[1]],
                                            ...
                                            X_test = test2,
                                            prior_params = {'mu' : 1, 'vega': 10, 'alpha': 0.000000001, 'beta': 2}, 
                                            objective='regression')      
    
 
    train_encode1, test_encode1 = conjugate_encoder.conjugate_encoder(
    X_train = train1, 
    y = train1.target_cont, cat_columns= ['feature_cat_chr','feature_cat_num'],
    X_test = test1,
    prior_params = prior_param_reg, 
    objective = 'regression') 
    ...
    X_train = train2, 
    y = train2.target_cont, cat_columns= ['feature_cat_chr','feature_cat_num'],
    X_test = test2,
    prior_params = prior_param_reg, 
    objective = 'regression') 
    ...
    # Test for correct values. Train/Test 1 is a standard data set, Train/Test 2 is a dataset where
    # test 2 contains values not found in train.
    
    assert pandas.testing.assert_series_equal(
        train_encode1['encoded_mean_feature_cat_chr'],
        true_train1_mean, 
        check_names=False, check_less_precise = True) == None, "Incorrect mean values being returned in training set 1."
    
    assert pandas.testing.assert_series_equal(
        train_encode1['encoded_var_feature_cat_chr'],
        true_train1_var, 
        check_names=False, check_less_precise = True) == None, "Incorrect variance values being returned in training set 1."
    
    assert pandas.testing.assert_series_equal(
        test_encode1['encoded_mean_feature_cat_chr'],
        true_test1_mean, 
        check_names=False, check_less_precise = True) == None, "Incorrect mean values being returned in test set 1."
    
    assert pandas.testing.assert_series_equal(
        test_encode1['encoded_var_feature_cat_chr'],
        true_test1_var, 
        check_names=False, check_less_precise = True) == None, "Incorrect variance values being returned in test set 1."
    
    assert pandas.testing.assert_series_equal(
        train_encode2['encoded_mean_feature_cat_chr'],
        true_train2_mean, 
        check_names=False, check_less_precise = True) == None, "Incorrect mean values being returned in training set 2."
    
    assert pandas.testing.assert_series_equal(
        train_encode2['encoded_var_feature_cat_chr'],
        true_train2_var, 
        check_names=False, check_less_precise = True) == None, "Incorrect variance values being returned in training set 2."
    
    assert pandas.testing.assert_series_equal(
        test_encode2['encoded_mean_feature_cat_chr'],
        true_test2_mean, 
        check_names=False, check_less_precise = True) == None, "Incorrect mean values being returned in test set 2."
    
    assert pandas.testing.assert_series_equal(
        test_encode2['encoded_var_feature_cat_chr'],
        true_test2_var, 
        check_names=False, check_less_precise = True) == None, "Incorrect variance values being returned in test set 2."
...
def test_check_binary():
    ...
    train_encode2, test_encode2 = conjugate_encoder.conjugate_encoder(
    X_train = train2, 
    y = train2.target_bin, cat_columns= ['feature_cat_chr','feature_cat_num'],
    X_test = test2,
    prior_params = prior_param_class, 
    objective = 'binary') 
    ...
    assert pandas.testing.assert_series_equal(
        ...
        check_names=False, check_less_precise = True, check_index_type=False) == None, "Incorrect mean values being returned in training set 1."
